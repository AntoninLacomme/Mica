...
class MicaServer {
    ...
    // webPath -> le path dans l'URL du navigateur
    // toDo -> la fonction de traitement, return true ou false
    // redirectTrue -> le path de la page sur laquelle il faut rediriger si le toDo a réussi
    // redirectFalse -> le path de la page sur laquelle il faut rediriger si le toDo a échoué
    createPostRountingPath (webPath, redirectTrue, redirectFalse, toDo) {
        this.app.post (webPath, (req, res) => {
            if (toDo (req, res)) {
                res.redirect (redirectTrue)
            } else {
                res.redirect (redirectFalse);
            }
        });
    }
    ...
    // fonction permettant de définir une page web par requête HTTP POST
    // webPath -> le path dans l'URL du navigateur
    // toDo -> la fonction de traitement
    createPost (webPath, toDo) {
        this.app.post (webPath, (req, res) => {
            let data = toDo (req, res)
            res.send (data);
        });
    }
}
